...
async function sendBurnScheduleFileToTelegram() {
  ...
  try {
    ...
    form.append("caption", "📄 *GTG DAILY BURN AND AIRDROP SCHEDULE*\n\nDownload the full burn plan.");
...
console.log("🔥 Burning", BURN_AMOUNT_UI, "tokens...");
    ...
    if (AIRDROP_TOTAL_UI > 0 && MIN_HOLD_UI > 0) {
      ...
      if (remainingBaseUnits < airdropTotalBase) {
        console.log("⚠️ Not enough balance for airdrop of", AIRDROP_TOTAL_UI, “GTG; skipping airdrop.");
      } else {
        console.log("🔎 Fetching all GTG holders via getParsedProgramAccounts...");
...
const scheduleMsg =
  "📄 *GTG DAILY BURN AND AIRDROP SCHEDULE*\n" +
